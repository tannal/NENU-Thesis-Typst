...
为使 GPT2 模型能够有效理解和生成 ABC 记谱文本，本文设计并实现了专门面向 ABC 音乐语料的 ABCTokenizer。该分词器根据 ABC 记谱法的语法特点，将乐曲中的音高（如 C, D'）、时值（如 1/8）、调性与节拍标记（如 K:G, M:4/4）、装饰音与和弦符号，以及小节线和结构标记等切分为离散的子词单元，并构建对应的词表。通过这种针对性设计，ABCTokenizer 能够在尽量保留乐谱结构信息的前提下，减少稀有符号和长序列造成的建模负担，为 GPT2ABC 提供更语义一致、结构清晰的输入序列，从而提升模型对旋律模式与节奏结构的学习能力。

== GPT2ABC

GPT2ABC 是本文构建的基于 GPT-2 架构的 ABC 音乐生成模型。模型在预训练 GPT-2 的基础上，引入面向 ABC 记谱文本的专用分词器 ABCTokenizer，将乐谱中的音符、时值、调性、节拍及结构标记等编码为离散序列，再通过多层自注意力机制建模其长距离依赖与乐句结构。GPT2ABC 采用自回归生成方式，在给定起始片段或风格条件的情况下，逐步预测后续 token，从而生成旋律连贯、节奏稳定且结构完整的 ABC 乐谱。实验结果表明，相较 RNN、LSTM 与 Transformer 等基线模型，GPT2ABC 在困惑度与测试集损失上均取得更优表现，说明其在学习 ABC 音乐语言和捕捉高层次音乐结构方面具有更强的建模能力。
